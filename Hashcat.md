...
## Intro
Sometimes we can even use something like [CrackStation - Online Password Hash Cracking - MD5, SHA1, Linux, Rainbow Tables, etc.](https://crackstation.net/) or  [Best MD5 SHA1 Decrypt | Encrypt | Crack | Decode | Hash Toolkit](https://hashtoolkit.com/) before
attempting to use Hashcat

## How to identify hashes though?
Kali comes with two different tools. The first one is more precise, but the second one already outputs the hashcat command, which is handy.
...
`hashcat -D 2 -m 0 Hash/hash.txt /usr/share/wordlists/rockyou.txt`
